...
export default function AIAnalysisPanel({ analyses }: { analyses: EngineOutput[] }) {
    ...
    const [selected, setSelected] = useState<EngineOutput | null>(
        safeAnalyses.length > 0 ? safeAnalyses[0] : null
    )

    if (safeAnalyses.length === 0) {
        return <div className="text-center py-20 text-slate-500 text-sm">No analysis available.</div>
    }
                    ...
                    <div className="flex items-center justify-between">
                        <div>
                            ...
                            <p className="text-slate-400 text-xs">
                                {selected.direction.toUpperCase()} · {selected.trend} · {selected.marketBias}
                            </p>
                        </div>

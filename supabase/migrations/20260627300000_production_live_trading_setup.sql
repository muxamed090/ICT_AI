...
CREATE INDEX IF NOT EXISTS broker_accounts_user_id ON public.broker_accounts(user_id);
...
CREATE TABLE IF NOT EXISTS public.trade_queue (
  id                 UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id            UUID        NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS trade_queue_user_status ON public.trade_queue(user_id, status);
ALTER TABLE public.trade_queue ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users manage own trade queue" ON public.trade_queue FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE INDEX IF NOT EXISTS broker_orders_user_id ON public.broker_orders(user_id);
ALTER TABLE public.broker_orders ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users manage own broker orders" ON public.broker_orders FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
...
CREATE INDEX IF NOT EXISTS broker_positions_user_id ON public.broker_positions(user_id);
ALTER TABLE public.broker_positions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users manage own broker positions" ON public.broker_positions FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- 6. Broker Logs Table -- live audit trails
CREATE TABLE IF NOT EXISTS public.broker_logs (
  id                  UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id             UUID        NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  broker_account_id   UUID        REFERENCES public.broker_accounts(id) ON DELETE SET NULL,
...
CREATE INDEX IF NOT EXISTS broker_logs_user_id ON public.broker_logs(user_id);
ALTER TABLE public.broker_logs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Users view own broker logs" ON public.broker_logs FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
